...
def getPosicao(gps):
    ...
    #O gpsd mostra as informações sendo recebidas pelo GPS em formato de lista, para que seja possível retirar informações dele é criada a varíavel px, que serve para passar para a próxima linha.
    px = gpsd.next()

    #Faz-se uma checagem se o gpsd retornou a classe 'TPV', significando que o GPS conseguiu um fix, ou seja, conseguiu fixar sua localização com três satélitese portanto está pronto para repassar as informações recebidas.
    if px['class'] == 'TPV':
...
def chamaBussola():
    ...
    #Aqui se cria a instância do magnetômetro, definindo para ele sempre ficar ativo. Define-se também, no setDeclination a inclinação em graus e minutos da onde será executado o código, para que a bússola seja precisa. Será ocultado por motivos de segurança, mas você pode checar a inclinação do seu local  neste site: https://www.magnetic-declination.com/
...
def segueCone(capture):
    ...
    #Foram tiradas duas cores do laranja: uma onde a luz do ambiente estava acesa e a outra com ela apagada, representada pelo LA e LD respectivamente. No momento está sendo usado somente a LA, mas é possível juntar os 4 ranges em uma máscara só e usá-la.
...
coneLonge = True
    x += 1
    print("ta indo")
    #Esse while é necessário para a ativação do GPS, onde serve como uma forma de aguardar o fix ao mesmo tempo que retira as informações dele para que possam ser checadas na tela.
...
# É definida uma aceitação de erro para que o robô encontre a coordenada da qual ele tem que ir, pensando tanto em uma possível demora para o robô parar, assim como um possível delay no magnetometro.
...
while i != 3:
    ...
    for contours in contorno:
        ...
        if areaContorno > 1500:
            #Caso a area contornada seja maior que 1500px, pega os dados da onde está sendo feito o contorno, para que se possa descobrir a onde o cone está em relação ao robô, esquerda, direita ou meio.   
            ...
            #Quando o cone está colado na câmera, a área do contorno fica em torno de 4800 a 5200. Como se faz necessário que o robô pare antes, é definido um range menor.
    ...
    while coneLonge:
        while procurandoAngulo:
            ...
            #Recebemos da bussola para onde o robô está indo, pegando o primeiro termo pois o magnetometro sempre retorna graus e minutos, mas aqui só importa os graus.
            ...
            if anguloAtual > direcaoGraus:
                ...
                escDir.ChangeDutyCycle(0)
                #Não precisa que o angulo que o robô está agora seja exatamente o angulo que está a coordenada, portanto, colocamos-o dentro do range definido anteriormente.
